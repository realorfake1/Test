...
local Tabs = {
	Author = Window:AddTab({Title = "Author", Icon ="user-round-cog"}),
...
_G.FastAttack = true    
local AuthorScript = Tabs.Misc:AddSection("Author")
Tab.Author:AddButton({
	Title = "Facebook",
	Description = "Tại Đây",
    Callback = function()
...
if Third_Sea then
    local RoughSea = Tabs.Main:AddSection("Sea Events")
    ...
    local ToggleShip = Tabs.Main:AddToggle("ToggleShip", {Title = "Auto Ship", Default = false })
...
ToggleRemoveNotify:OnChanged(function(Value)
    ...
    Options.TogglrAutoT:SetValue(false)
    spawn(function()
    	while wait() do
    	    pcall(function()
    	        if _G.AutoT then
